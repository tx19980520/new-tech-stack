...
TLS 在 web 端的使用非常广泛，能够对传输的内容进行加密，从而有效的防止中间人攻击。双向 TLS（Two way TLS/Mutual TLS，后文均简称 mTLS）的主要应用于 B2B 和 Server-to-Server 场景，以支持服务与服务之间身份认证与授权。
![istio-official-authorization-arch](../images/authz.png)

上图源引[Istio官网](https://istio.io/docs/concepts/security/authz.svg)，表明 Istio 希望能在服务网格中使用 mTLS 进行授权，而在服务网格外使用 JWT+mTLS 进行授权。在服务间身份认证中使用 mTLS，在来源身份验证中使用 JWT。
...
为验证 mTLS 实现服务间授权，实验环境将被沿用，主要用于验证在服务网格内部的进行通信时，如何使用服务间授权来进一步细粒度保护相应服务。
...
当 `sleep.full` 请求 `httpbin.mtls-test` 时，返回403，这说明该请求携带证书，但是证书的的 SAN 值域并不在 Trust Domain 中，因此返回403以表示访问权限不足。
